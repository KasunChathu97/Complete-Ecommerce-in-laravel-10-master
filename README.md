...
```sh
[git clone https://github.com/KasunChathu97/Complete-Ecommerce-in-laravel-10-master.git
...
### 🔹 SMS + Email notifications from SMS Logs
Whenever a new row is inserted into the `sms_logs` table (for example when an order is placed/updated), the system will:
- Email the admin + customer
- Send SMS to the customer and also to the admin phone (via API)
...
```
Import `database/DL.sql` into your database manually (if needed).
